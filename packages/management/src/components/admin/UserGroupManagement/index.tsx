...
import { getGroupsByPages, getUsersByPage } from "../../../api/admin";
  ...
  return (
    <div>
      <Table
        ...
        dataSource={groups || []}
        rowKey="id"
        onExpand={(expanded: boolean, record: { id: string; name: string; childrens?: any[] }) => {
          if (expanded !== undefined && record) {
            return handleExpand(expanded, record);
          }
          return false;
        }}
        expandedRowRender={(record: { id: string; name: string; childrens?: any[] }) => (
          <Table
            columns={[
              {
                title: "用户名",
